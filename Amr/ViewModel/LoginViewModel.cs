...
namespace Amr.ViewModel
{
    public class LoginViewModel : INotifyPropertyChanged
    {
        ...
        private async Task login(object obj)
        {
            ...
            try
            {
                HttpResponseMessage response = await client.PostAsJsonAsync("http://localhost:5001/api/Account/login", dto);

                if (response.IsSuccessStatusCode)
                {
                    var token = await response.Content.ReadAsStringAsync();
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                    var res = await client.GetAsync("http://localhost:5001/api/EnergyMeters");
